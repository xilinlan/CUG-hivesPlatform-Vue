      ...
      <el-col :span="8">
        <el-card style="width: 100%; min-height: 300px; color: #333">
          ...
          <div style="padding: 10px 0" v-for="user in users" :key="user.nickname">
            ...
            <el-button @click="chatUser=user.nickname">choose</el-button>
  ...
  methods: {
    ...
    init() {
      ...
      if (typeof (WebSocket) == "undefined") {
        ...
        console.log("您的浏览器支持WebSocket");
        let socketUrl = "ws://localhost:14000/chat/" + nickname;
        if (socket != null) {
          socket.close();
          socket = null;
        }
        ...
        socket.onmessage = function (msg) {
          ...
          let data = JSON.parse(msg.data)  // 对收到的json数据进行解析， 类似这样的： {"users": [{"nickname": "zhang"},{ "nickname": "admin"}]}
